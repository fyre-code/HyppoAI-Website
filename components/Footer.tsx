    ...
    <footer className="bg-[#0a0a0a] border-t border-[#1a1a1a] pt-16 pb-8">
      <div className="max-w-[1170px] mx-auto px-6">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-12 mb-12">
          {/* Brand */}
          <div>
            <Image src="/logo.png" alt="HyppoAI" width={480} height={144} className="h-32 w-auto mb-4" />
          </div>
          ...
          <div className="flex flex-col items-center justify-center gap-4">
            {[
              ...
              { label: 'LinkedIn', href: 'https://www.linkedin.com/in/brandongosselin2032/', icon: 'M16 8a6 6 0 016 6v7h-4v-7a2 2 0 00-2-2 2 2 0 00-2 2v7h-4v-7a6 6 0 016-6zM2 9h4v12H2z M4 6a2 2 0 100-4 2 2 0 000 4z', fill: false },
              ...
              <a key={label} href={href} target="_blank" rel="noopener noreferrer" aria-label={label} className="flex items-center gap-2 text-[#555] hover:text-[#2cd9fe] transition-colors">
                <svg className="w-6 h-6 shrink-0" fill={fill ? 'currentColor' : 'none'} stroke={fill ? 'none' : 'currentColor'} strokeWidth={1.5} viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" d={icon} />
                </svg>
                <span className="text-[#2cd9fe] text-sm">{label}</span>
              </a>
            ))}
          </div>
